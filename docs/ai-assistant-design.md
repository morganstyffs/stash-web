...
- **ผู้ใช้ต้องเห็นชัดว่ายินยอมอะไร ก่อนกดเปิด** — ข้อความสั้น ภาษาคน เช่น
  *"เมื่อเปิด ระบบจะส่งคำถามของคุณและตัวเลขที่เกี่ยวข้อง (เช่น ยอดในเดือนที่ถาม) ไปประมวลผลที่ผู้ให้บริการ AI
  (Anthropic) เพื่อสร้างคำตอบ · ปิดได้ทุกเมื่อ"* — วางในกลุ่ม "ผู้ช่วย AI" ของ `SettingsPage`
- **ปิด toggle แล้วเกิดอะไร:** (client) ปุ่ม "ถาม AI" หาย + ไม่เรียก `/api/ai` · (server) แม้มีคนยิงตรง
  worker เช็ค consent=false → **คืน 403 + ข้อความไทย** *"ยังไม่ได้เปิดใช้ผู้ช่วย AI — เปิดได้ในหน้าตั้งค่า"*
...
| PR | เรื่อง | migration? |
|---|---|---|
| **PR-0** | 🔴 **ตาราง consent `ai_settings`** (`user_id pk`, `consent boolean not null default false`, `updated_at`) · RLS owner-only CRUD · ดึง types ผ่าน types-drift **พร้อม** แก้ fixture ที่ประกอบแถวใหม่ให้คอมไพล์ได้ (atomic — กับดัก `opening_balance`/§2.1) · **ยังไม่มี reader/writer** (column default false = ทุกคน consent=false = fail closed จนกว่า PR-1/PR-4 มา) | ✅ **ต้องมี** |
| **PR-1** | **ด่านความปลอดภัยของ `/api/ai`** — verify ด้วย `auth.getUser` (401) · **เช็ค consent จาก `ai_settings` (403)** · สร้าง Supabase client ต่อ-request แนบ JWT ผู้ใช้ (ไม่ใช่ service_role) · **KV namespace** + rate limit ผูก uid (429) · ลำดับบังคับ verify→consent→limit→Anthropic · error ไทยครบ (401/403/429/502/504) · ต่อ end-to-end กับ **tool อ่านอย่างเดียว 1 ตัว** (`wallet_balances`) พิสูจน์เส้นทาง · **ขึ้นกับ PR-0** | ❌ (แก้ `wrangler.jsonc` เพิ่ม KV binding) |
...
| **PR-3** | 🔴 **RPC ใหม่ `stock_intake_list(p_from,p_to)`** (INVOKER · RLS · คืน count จริงทั้งชุด + รับ limit ตาม §4.1) + smoke test ในหัวไฟล์ + ดึง types ผ่าน types-drift **พร้อม** call site (§2.1 — ห้าม merge types-drift เดี่ยวเมื่อมี call site) + tool `stock_intake` | ✅ **ต้องมี** |
| **PR-4** | **UI** — ถอด `disabled` ปุ่ม "ถาม AI" 2 จุด + หน้า/overlay แชท + **consent writer + ข้อความยินยอมภาษาคน** (§3.5) ต่อ toggle "ใช้ผู้ช่วย AI" ↔ `ai_settings.consent` + error surfacing (`errors.ts`) + hideBalance ตอบเลขจริง + ประวัติ ephemeral | ❌ |
| **PR-5** (optional) | ประวัติแชทถาวร — ตาราง `ai_chats` RLS owner-only · **⚠️ ต้องตัดสิน mask ประวัติเก่าใหม่:** ประวัติที่เต็มไปด้วยตัวเลขถูกเรนเดอร์ตอนเปิด `hideBalance` = "กวาดตา" เต็ม ๆ ขัด §11.4-11 → **ห้ามยกคำตัดสิน "ตอบเลขจริง" ของ v1 มาใช้ต่อ** (v1 ยอมได้เพราะ ephemeral หายเมื่อ reload) | ✅ ถ้าทำ |

**ใบที่ต้องมี migration: PR-0 (บังคับ) · PR-3 (บังคับ) · PR-5 (ถ้าทำ)** · ที่เหลือ client/worker ล้วน
**ลำดับ:** PR-0 → PR-1 → (PR-2a → PR-2b) → PR-3 → PR-4 · consent (PR-0) มาก่อนสุดเพราะ endpoint ต้องเช็คได้ก่อนทำงาน
...
> เดี่ยวปลอดภัยตามข้อยกเว้น §2.1 (ต้องมัด types+fixture atomic เท่านั้น) (3) แยกแล้ว PR-1 (worker) กับ PR-4 (UI)
> อ้างตารางที่มีอยู่จริงได้เลย ไม่ต้อง stack schema+worker+UI ในใบเดียว · default=false ทำให้ระหว่างทาง fail closed
> (ทุกคน consent=false จนกว่า UI เปิดให้กด) ไม่มีช่วงไหนที่ endpoint ทำงานโดยไม่มี consent
...
8. **ถ้าเก็บประวัติแชทภายหลัง (PR-5)** = สำเนาข้อมูลเงิน at-rest เพิ่มที่ต้องปกป้อง (RLS เดียวกัน แต่พื้นผิวโต) ·
   **และต้องตัดสิน mask ตัวเลขเก่าตอน `hideBalance` ใหม่** (§9 PR-5) — ห้ามยกคำตัดสิน v1 มาใช้
