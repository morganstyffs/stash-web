...
| F-31 | ✅ Resolved (🔴 หลุด production) | B/J | **`qty_remaining` ambiguous ใน stock_sale RPC** — `stock_sale_create`/`stock_sale_reverse` ประกาศ `qty_remaining` เป็น `RETURNS TABLE` column (= ตัวแปร plpgsql) แล้ว `select ... qty_remaining ... from stock_items` **ไม่ qualify** → runtime error `42702` "column reference qty_remaining is ambiguous" · **กดขายไม่ได้เลยบน production** (POST rpc/stock_sale_create → 400) · error ประเภทนี้ไม่เกิดตอน `create function` และผ่านทุก verification แบบ "ฟังก์ชันมีอยู่/granted" → หลุดถึง prod แม้ migration + review ผ่าน | `0012_stock_sale.sql` (SECTION 9,10) → แก้ใน `0013_fix_sale_ambiguous.sql` | ขาย/ย้อนการขายพังทั้งหมด | **แก้ใน 0013:** reproduce ทั้ง 2 ฟังก์ชันจาก 0012 เปลี่ยนเฉพาะ qualification (alias ทุกตาราง + qualify ทุกคอลัมน์ทุก statement) logic เดิมเป๊ะ (diff ยืนยัน) · signature เดิม → `create or replace` · smoke test ในหัวไฟล์พิสูจน์ว่า**รันได้** (bogus id → 'stock item not found' ไม่ใช่ ambiguous) · audit ทั้ง 0012/0011: `stock_sales_summary`/`stock_intake_create`/`stock_sku_build`/`stock_sku_preview` ปลอดภัย (Convention 21) |
| F-30 | ✅ Resolved | G/F | **ไม่มีหน้ากู้รหัสผ่าน** — ลืมรหัสทีต้อง reset ให้ใน dashboard เอง (บล็อกการเปิดให้เพื่อนใช้) | `ForgotPasswordPage.tsx`, `ResetPasswordPage.tsx`, `useAuth.tsx`, `auth.ts`, `router.tsx`, `LoginPage.tsx` | — | **แก้ใน PR D:** Supabase reset flow — `/forgot-password` (ส่งอีเมล) + `/reset-password` (รับ recovery session ผ่าน detectSessionInUrl → `updateUser({password})`) · **enumeration-safe** (Convention 17): ข้อความหลังส่ง = neutral "ถ้าอีเมลนี้มีบัญชี…" เสมอ · **recovery-only gate:** ฟอร์มตั้งรหัสโชว์เฉพาะเมื่อมี `PASSWORD_RECOVERY` event (`isRecovery`) ไม่ใช่แค่มี session — กัน session ปกติ (มือถือที่ปลดล็อกอยู่) เปลี่ยนรหัสโดยไม่รู้รหัสเดิม · detectSessionInUrl ล้าง token ออกจาก URL อัตโนมัติ · ล็อกด้วย `auth.test.ts` (10 เคส) · ต้องตั้งค่า dashboard (redirect URL allowlist) — ดู checklist ใน PR |
...
21. **plpgsql ที่มี `RETURNS TABLE` / `OUT` param → alias ทุกตาราง + qualify ทุกคอลัมน์ในทุก statement** — ชื่อ OUT column กลายเป็นตัวแปรในสโคป ถ้าตรงกับชื่อคอลัมน์จริงแล้วอ้างแบบไม่ qualify จะ **ambiguous (`42702`) ตอน runtime เท่านั้น** — `create function` ผ่าน, verification แบบเช็คการมีอยู่ผ่าน, แต่พอ**เรียกจริงพัง** (F-31 หลุด production: กดขายไม่ได้) · verification ต้อง**พิสูจน์ว่ารันได้** (smoke test เรียกฟังก์ชันจริง) ไม่ใช่แค่ "ฟังก์ชันมีอยู่/granted"
