...
| F-14 | ✅ Resolved | G | **`recurring_run_due` error กลืนเงียบถาวร** (เดิม) | `useRecurring.ts`, `errors.ts` | — | **แก้ใน PR A:** แยกเฉพาะ "function not found" (`isMissingFunctionError` = PGRST202/42883) ให้เงียบต่อ (migration ยังไม่รัน); error อื่นทั้งหมด → `toast.error('รายการประจำไม่ทำงาน: …')` ผ่าน `translateError` |
