...
**งานใหญ่ถัดไปที่ยังเปิดอยู่:** UI หนี้เพื่อน — **หน้ายอดค้าง/เพิ่มเพื่อน/โปรไฟล์ (PR-W) + สร้าง/ยืนยัน/ปฏิเสธ/ประวัติรายคน (PR-X) ลงแล้ว** · เหลือ **เคลียร์ยอด (`debt_settle`) + ย้อนการเคลียร์ + แถวล็อกในประวัติ + กวาดข้อความ error "หนี้" ในชุด RPC 0015 = PR-Y** · แท็บ `รอจ่าย` ออกแบบ `PendingItem` (วันที่·ป้าย·เงิน·ไอคอน·`source`) เผื่อรับ `source:'debt'` ไว้แล้ว
