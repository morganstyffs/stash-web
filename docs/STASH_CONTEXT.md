...
- **ประวัติแชท "ถาวร" (persistence) ยังไม่ทำ** — แชท **multi-turn ในหน่วยความจำ** แล้ว (ส่ง transcript กลับทุกครั้ง · §11.9) แต่ **ยังไม่เขียน localStorage/DB · reload = ล้าง** · **ติดเรื่อง mask `hideBalance`:** คำตัดสิน "แชทตอบเลขจริงแม้เปิด `hideBalance`" (ถามเจาะจง) ยกไปใช้กับประวัติถาวร (กวาดตา) ไม่ได้ ต้องเคาะ mask ใหม่ก่อนทำ
...
- **แชทตอบเลขจริงแม้เปิด `hideBalance`** — เพราะเป็นการถามเจาะจง ไม่ใช่กวาดตา (§11.4-11) · `AiPage` ไม่รับ prop ใด ๆ → `hideBalance` ส่งเข้าไม่ได้เชิงโครงสร้าง · **ยกไปใช้กับประวัติถาวรไม่ได้** (ประวัติถาวร = กวาดตา · §10)
- **ประวัติแชท ephemeral ในหน่วยความจำ (ยังไม่ persist)** — `AiPage` เก็บใน `useState` เคลียร์เมื่อ reload · ไม่เขียน localStorage/DB · **แต่ตอนส่งจะแนบ transcript ก่อนหน้ากลับไป (multi-turn)** โดย map ทีละ field เป็น `ChatTurn[]` (annotation กัน field แปลกหลุดขึ้น wire) · token อ่านสดจาก session ทุกครั้ง ไม่ cache · กันกดซ้ำ (ปุ่ม disabled + guard `pending`) · error ถึงผู้ใช้ผ่าน `translateError` (map ตาม HTTP status) — คำถามคาไว้ใน transcript ให้เห็นว่าอะไรล้ม
