...
| **`0020` เปลี่ยน signature RPC ที่ client เรียกอยู่ (`friend_request_send` `p_code`→`p_username`) แล้ว merge PR `types-drift` เดี่ยว → types ใหม่ + call site เก่าไม่ตรง → `tsc` ล้ม → main แดง → ต้องแก้สามใบ (types → hotfix → client)** | migration ที่เปลี่ยน/เพิ่ม signature RPC ที่ client เรียก **ห้าม merge PR `types-drift` เดี่ยว** · ดึงไฟล์ `database.types.ts` จาก branch ของ PR นั้นเข้า branch ฟีเจอร์ (`git checkout origin/<types-branch> -- src/lib/database.types.ts`) แล้ว **merge ทีเดียวพร้อม call site** · PR `types-drift` จะเงียบไปเองรอบถัดไปเพราะไฟล์ตรงแล้ว · (`0021` ทำแบบนี้แล้ว = ใบเดียวจบ) |
...
11. **`hideBalance` = "ซ่อนตอนกวาดตา เปิดตอนตัดสินใจ"** — ปิดยอดในลิสต์/พาดหัว/รายคน (กวาดตา) ได้ · แต่ sheet ที่ขอให้ผู้ใช้ยอมรับข้อผูกพัน (ยืนยันยอดค้าง — `ConfirmDebtSheet` · เคลียร์ยอดใน PR-Y) **ต้องแสดงจำนวนเงินเสมอ** ไม่ว่า `hideBalance` เปิดหรือไม่ · ยืนยันยอดที่ถูกปิดอยู่ = ยอมรับข้อผูกพันแบบตาบอด ไม่ใช่การซ่อนยอด (มีคอมเมนต์กำกับที่หัวไฟล์ `ConfirmDebtSheet.tsx`)
12. **ค้นหาเพื่อนใช้ `profiles.username` (0020) ไม่ใช่ `friend_code`** — username ตั้งเอง (พิมพ์เล็ก `a-z0-9_` 3–20) ตั้งครั้งเดียว (trigger กันแก้ · owner แก้ให้ได้ผ่าน SQL Editor เพราะ `auth.uid()` เป็น null) · **หนี้ที่รู้ตัว:** คอลัมน์ `friend_code` + `generate_friend_code()` **ยังอยู่แต่เลิกใช้แล้ว** (ไม่ drop เพื่อเลี่ยง migration destructive) ไม่มี code path อ่าน · PR ภายหลังค่อย drop เมื่อมั่นใจว่าไม่มีอะไรพึ่ง
13. **ย้อนการเคลียร์ยอด (`debt_settle_reverse`) ได้เฉพาะคนที่กดเคลียร์เอง** (`settled_by = auth.uid()`, 0015 §12) — ข้อจำกัดที่ตั้งใจ: ถ้าอีกฝ่ายไม่เห็นด้วยว่าจ่ายแล้ว ทำอะไรในแอปไม่ได้ ต้องคุยกันข้างนอก · ปลอดภัยกว่าให้ใครก็ได้ย้อน/ลบรายการเงินของอีกฝ่าย · UI โชว์ปุ่มย้อนเฉพาะแถวที่ "เราเป็นคนกดเคลียร์" · **แถวที่ล็อก** (ขายสต็อก + เคลียร์ยอด) รวมเป็นแนวคิดเดียวที่ `lib/ledger.ts` `lockedRowInfo()` — ล็อกไหม/เพราะอะไร/ไปย้อนที่ไหน · เพิ่มประเภทใหม่แก้ที่เดียว

**งานใหญ่ถัดไปที่ยังเปิดอยู่:** UI หนี้เพื่อน — **ครบวงจรแล้ว (PR-W เพิ่มเพื่อน/โปรไฟล์ · PR-X สร้าง/ยืนยัน/ปฏิเสธ/ประวัติ · username 0020 · PR-Y เคลียร์/ย้อน/แถวล็อก)** · เหลือ **กวาดข้อความ error "หนี้" ในชุด RPC `0015` (reproduce ทั้งชุดเปลี่ยน literal) = migration เดี่ยวทีหลัง** · แท็บ `รอจ่าย` ออกแบบ `PendingItem` เผื่อรับ `source:'debt'` ไว้แล้ว
