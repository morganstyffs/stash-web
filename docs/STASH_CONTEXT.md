...
- **ไม่มี "ราคาขายเป้าหมาย" (`target_price`) แล้ว** (0027) — **ราคาขายกรอกตอนขายเท่านั้น** (แผงขายใน `StockEditSheet` · เปิดมาว่าง ไม่เติมค่าล่วงหน้า) · เดิมเก็บราคาที่ "หวัง" แล้วคูณโชว์ "รอขาย/กำไรคาดการณ์" = ตัวเลขที่ดูเหมือนเงินแต่ไม่ใช่เงิน · ที่ว่างบนหน้าคลังใส่ **ทุนจม** แทน = ต้นทุนของที่ยังไม่ขายหมด **และค้างเกิน `AGE_OLD_MAX` (60) วัน** (`StockPage.computeSunkCost` · reuse `isStale` — เกณฑ์อายุ**ที่เดียว**กับชิป "ค้างนาน" ไม่ประกาศเลข 60 ซ้ำ) · เป็นเงินจริง → mask ตาม `hideBalance` · ไม่มีของค้าง → โชว์ข้อความ "ไม่มีของค้างนาน" ไม่ใช่ ฿0 (§11.4-31/32)
- การ์ดสินค้าโชว์ **ต้นทุน + จำนวนวันในคลัง** (`฿2,500 · 45 วัน`) ไม่ใช่ลูกศรราคา · วันนับจากวันที่รับเข้า (`daysSince(created_at)` · คอลัมน์เดียวกับชิป "ค้างนาน") · ของที่ขายหมดไม่โชว์วัน · จำนวนวัน**ไม่** mask (ไม่ใช่จำนวนเงิน)
...
**ไฟล์ migration ในสาขานี้: `0001`–`0027`** (ล่าสุด `0027_drop_target_price.sql`) — ทุกฟังก์ชัน/ตารางใน `database.types.ts` มีไฟล์ migration รองรับครบ (ตรวจ cross-check แล้ว ไม่มี "อยู่ใน types แต่ไม่มีไฟล์")
`0027` = **ตัดราคาขายเป้าหมาย (`target_price`)** — DROP คอลัมน์ `stock_items.target_price` (CHECK `stock_items_target_nonneg` จาก 0009 หายเองพร้อมคอลัมน์) · reproduce `stock_intake_create` ตัด `p_target_price` (14→13 args · ยกตัวนับ atomic/retry/Bangkok-date จาก 0025 ทั้งดุ้น) · drop ด้วย 14-arg signature จริง แล้ว re-grant 13-arg · แทนที่ "รอขาย/กำไรคาดการณ์" บนหน้าคลังด้วย **ทุนจม** ฝั่ง client (§5 · §11.4-31/32)
...
**เทสต์ (จากการรัน `npm test` จริง หลัง 0027): 375 เคส / 42 ไฟล์ — ผ่าน 370 · skip 5** (5 ที่ skip = visual guard นอก CI ตามปกติ · §9) · หมายเหตุ: `npm run build` (tsc) จะแดง 3 จุดที่ test factory ตัด `target_price` จน `database.types.ts` ถูก regen หลัง apply 0027 (CI-red-until-owner-acts ตามแผน PR-T3)
...
- **การ์ดสรุปกำไรร้านในหน้าคลัง (T2)** — ยอดขาย − ต้นทุนที่ขายไป = กำไรขั้นต้น (ถังที่ 1) − ค่าดำเนินร้าน (ถังที่ 2) = กำไรสุทธิ · สลับ **เดือนนี้ / 3 เดือน** (`SegmentedControl.tsx`) · สูตรอยู่ที่ **`lib/shopAccount.ts` `computeShopProfit`** (บ้านเดียว ไม่คิดใน component) · ถังที่ 1 = `useStockSalesRange` (RPC เดิม ไม่แตะ) · ถังที่ 2 = `useShopOperatingSummary` (รวมฝั่ง client, มี guard เพดานแถว) · ช่วง 3 เดือน = `trailingMonthsBounds` (`dates.ts`) · **หน้าคลังเคารพ `hideBalance` แล้วทั้งหน้า** (STOCK VALUE + ทุนจม + การ์ดกำไร + ต้นทุนบนการ์ดสินค้า + ปุ่มตา · จำนวนวันบนการ์ดไม่ mask) — §11.4-28/29 · **ไม่มี migration**
...
- **เกณฑ์ 60 วันของ "ทุนจม"/"ค้างนาน" (`AGE_OLD_MAX`) ยังเป็นตัวเลขที่เดา** — โค้ดเขียนคอมเมนต์ยอมรับเองว่าเป็นค่าประมาณสำหรับเสื้อผ้ามือสอง ไม่ได้วัดจาก turnover จริง (0027 · `StockPage.tsx`) → **ควรทบทวนหลังใช้จริงสัก 3 เดือน** ว่าปรับ 60 ขึ้น/ลง หรือแยกเป็นหลายชั้นอายุ (ยังไม่มีข้อมูลจริงให้ปรับตอนนี้)
...
**— ตัดสินใจรอบตัดราคาเป้าหมาย + ทุนจม (0027 · PR-T3) —**

31. **ตัดราคาขายเป้าหมายและกำไรคาดการณ์ทิ้ง — แทนด้วยทุนจม** — `target_price` คือราคาที่เจ้าของ "หวัง" ไม่ใช่ราคาที่ตลาดยอมจ่าย · เดิมถูกคูณโชว์ "รอขาย +฿X · ถ้าขายได้ตามราคาตั้ง" บนหน้าคลัง = **ตัวเลขที่ดูเหมือนเงินแต่ไม่ใช่เงิน** ทำให้รู้สึกรวยกว่าจริงตลอดเวลา — อันตรายเป็นพิเศษกับคนขายมือสองที่มัก "กำไรดีแต่เงินสดหมด" มากกว่าขาดทุน · แทนที่ด้วย **ทุนจม** = ต้นทุนจริงที่กองอยู่ในของค้างเกิน `AGE_OLD_MAX` (60) วัน (`computeSunkCost` reuse `isStale` — เกณฑ์อายุที่เดียวกับชิป "ค้างนาน" ไม่ประกาศ 60 ซ้ำ) · ตอบว่า "เงินหายไปกองตรงไหน / หยุดซื้อเข้าได้แล้ว" ซึ่งกำไรคาดการณ์ไม่เคยตอบ · ฿0 ตรงนี้เป็นข่าวดี → โชว์ "ไม่มีของค้างนาน" ไม่ใช่เลข 0 · **ห้ามเอา `target_price`/กำไรคาดการณ์กลับมาไม่ว่าจะในรูปแบบใด**
32. **ห้ามเติมค่าราคาขายล่วงหน้าในแผงขาย** (`StockEditSheet` · 0027) — ช่องราคาขายเปิดมา**ว่างและ focus ไว้** ไม่เดาจาก `target_price` เดิม (ตัดทิ้งแล้ว) และห้ามเดาจากต้นทุน/ราคาขายครั้งก่อน · เหตุผล: ค่าที่เดาให้แล้วผู้ใช้กดผ่านโดยไม่ดู จะกลายเป็น**ราคาขายผิดที่ไหลเข้ากำไรทันที** — และกำไรที่ถูกต้องคือสิ่งที่ทั้งชุด T ทำมา · หลักการเดียวกับ "ไม่ prefill ยอดค่าส่ง" (§11.4-30/§3.5) · แผงขายยัง**แสดงจำนวนเงินเสมอ ไม่รับ prop `hideBalance`** (ชีตที่ขอให้ผู้ใช้ยอมรับข้อผูกพันต้องเห็นตัวเลข — §11.4-11)
