 ...
 * PURCHASES (inventory asset) but INCLUDES recognised COGS — under Model A,
 * COGS is a real expense; netted against the sale income it leaves only profit,
 * so `income − spending` stays correct without a separate COGS term.
 */
 ...
 * is excluded (a resale's cost of goods is not discretionary budgeted spending).
 */
export function isBudgetSpendingRow(r: LedgerRow): boolean {
  return isSpendingRow(r) && !r.is_stock_cogs
}
